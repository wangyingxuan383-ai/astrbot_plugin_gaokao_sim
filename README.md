...
一个深度模拟高三学习生活的 AstrBot 插件，v2.1.6 版本引入了 LLM AI 互动、体力/压力数值系统以及可视化战报。
...
1. **时间线**: 9月到次年6月，10个阶段。每月行动需求为 1-2 次（随机），最快约 2 天、最慢约 4 天完成一轮。
2. **学科类型**: 随机分配文科或理科。
...
- `/高考菜单` - 显示帮助
- `/高考调试 [操作]` - 管理员调试（清理CD/满精力/重置负面/跳月/设月份/加分）

## ⚙️ 配置说明

在 AstrBot 管理面板中可配置：
- **是否启用 LLM 功能**: 开启/关闭 AI 测验和动态剧情
- **管理员QQ列表**: 允许使用调试命令的QQ号列表
...
- **报告图片保留数量**: reports 目录最多保留的图片数量，超出自动清理，设为 0 关闭清理
- **自定义字体路径**: 用于成绩单/折线图的中文字体，留空自动寻找系统字体
